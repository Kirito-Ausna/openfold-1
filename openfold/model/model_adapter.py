...
from openfold.model.adapter import (
    AdapterShare,
    AdapterSeperate,
)
...
class AlphaFold_adapter(nn.Module):
    """
    Alphafold 2.
    ...
    def __init__(self, config):
        ...
        """
        super(AlphaFold_adapter, self).__init__()
        ...
        #####################################################################
        self.adapter_mode = 'seperate'

        if self.adapter_mode == 'share':
            self.adapter_share = AdapterShare(
                **config['adapter']
            )
        elif self.adapter_mode == 'seperate':
            self.adapter_seperate = AdapterSeperate(
                **config['adapter']
            )
        #####################################################################
    ...
    def iteration(self, feats, m_1_prev, z_prev, x_prev, _recycle=True):
        ...
        if self.adapter_mode == 'share':
            s, z, AdaEmb = self.adapter_share(MSAQuaryEmb=s, PairEmb=z, SeqEmb=SeqEmb)
        elif self.adapter_mode == 'seperate':
            s, z, AdaSeqEmb, AdaStrEmb = self.adapter_seperate(MSAQuaryEmb=s, PairEmb=z, SeqEmb=SeqEmb)
        ...
        ###################################################################
        if self.adapter_mode == "share":
            return outputs, m_1_prev, z_prev, x_prev, AdaEmb
        elif self.adapter_mode == "seperate":
            return outputs, m_1_prev, z_prev, x_prev, AdaSeqEmb, AdaStrEmb
        ##################################################################
    ...
    def forward(self, batch):
        ...
        for cycle_no in range(num_iters):
            ...
            with torch.set_grad_enabled(is_grad_enabled and is_final_iter):
                ...
                if self.adapter_mode == 'share':
                    outputs, m_1_prev, z_prev, x_prev, AdaEmb = self.iteration(
                        feats,
                        m_1_prev,
                        z_prev,
                        x_prev,
                    ...
                    outputs, m_1_prev, z_prev, x_prev, AdaSeqEmb, AdaStrEmb = self.iteration(
                        feats,
                        m_1_prev,
                        z_prev,
                        x_prev,
        ...
        if self.adapter_mode == 'share':
            return outputs
            # return outputs, AdaEmb
        elif self.adapter_mode == 'seperate':
            return outputs
            # return outputs, AdaSeqEmb, AdaStrEmb